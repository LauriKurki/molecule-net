...
def build_mask(
    ...
    mask = torch.zeros(1, 1, *sh, dtype=dtype)
    c = (sh[0]-1) / 2
    t = (c - margin/100.*c)**2
...
class MaskModule3D(EquivariantModule):
    ...
    def __init__(
        ...
        Performs a 3D element-wise multiplication of the input with a *mask* of shape :math:`S^n`, where :math:`n` is the
        ...
        Args:
            ...
            S (tuple): the shape of the mask and the expected inputs
        ...
        self.mask = torch.nn.Parameter(
                build_mask(S, margin=margin, sigma=sigma),
                requires_grad=False,
        )
