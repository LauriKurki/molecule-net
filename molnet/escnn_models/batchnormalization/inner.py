...
class InnerBatchNorm3D(EquivariantModule):
    ...
    def __init__(self,
        ...
        Batch normalization (3D input) for representations with permutation matrices.
        ...
        if self.in_type.gspace.dimensionality == 2:
            raise NotImplementedError('InnerBatchNorm3D is not implemented for 2D input. Check if input flattening works.')
        elif self.in_type.gspace.dimensionality == 3:
            bnorm_class = torch.nn.BatchNorm2d # we flatten spatial channels due to https://discuss.pytorch.org/t/batchnorm-4d-or-n-dim/144070/4
        else:
    ...
    def forward(self, input: GeometricTensor) -> GeometricTensor:
        ...
        for s, contiguous in self._contiguous.items():
            ...
            if contiguous:
                bn_in = input.tensor[:, indices[0]:indices[1], ...].view(b, -1, s, *shape)
                bn_in = bn_in.view(*bn_in.shape[:3], -1) # flatten spatial dimensions
                # if the fields were contiguous, we can use slicing
                output[:, indices[0]:indices[1], ...] = batchnorm(
                    bn_in
                ).view(b, -1, *shape)
            else:
                bn_in = input.tensor[:, indices, ...].view(b, -1, s, *shape)
                bn_in = bn_in.view(*bn_in.shape[:3], -1) # flatten spatial dimensions
